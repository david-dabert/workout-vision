 ...
 *
 * Open the URL on your phone (same WiFi), pick a video or image,
 * choose the exercise, type the rep count, hit Send.
 * The file lands on your Mac and the manifest updates.
 *
 * Usage:
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  video, img.preview { width: 100%; max-height: 200px; border-radius: 8px; margin-top: 8px; display: none; background: #000; object-fit: contain; }
</style>
</head>
<body>

<h1>Clip Collector</h1>
<p class="sub">Film a set or snap a photo, send it here. Files stay on your network.</p>
...
<div class="field">
  ...
  <input type="file" id="fileInput" accept=".mov,.mp4,.m4v,.webm,.jpg,.jpeg,.png,.heic,.heif,video/*,image/*">
...
<div class="row">
  <div class="field">
    <label>Reps <span id="repsNote">(you counted)</span></label>
    <input type="number" id="reps" min="0" max="100" placeholder="e.g. 8 (0 for images)" inputmode="numeric">
  </div>
  <div class="field">
    <label>View</label>
    <select id="view">
...
fileInput.addEventListener('change', () => {
  ...
  if (selectedFile) {
    ...
    isImage = selectedFile.type.startsWith('image/') || /\\.(jpg|jpeg|png|heic|heif)$/i.test(selectedFile.name);
    const objUrl = URL.createObjectURL(selectedFile);
    if (isImage) {
      videoPreview.style.display = 'none';
      imgPreview.src = objUrl;
      imgPreview.style.display = 'block';
      repsNote.textContent = '(optional for images)';
      reps.placeholder = '0 for static image';
    } else {
      imgPreview.style.display = 'none';
      videoPreview.src = objUrl;
      videoPreview.style.display = 'block';
      videoPreview.play();
      repsNote.textContent = '(you counted)';
      reps.placeholder = 'e.g. 8';
    }
    checkReady();
  }
});
...
sendBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    isImage = false;
    fileInput.value = '';
    fileBtn.textContent = 'Tap to select file';
...
const server = createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && req.url.startsWith('/upload')) {
    ...
    const reps = parseInt(url.searchParams.get('reps'), 10) || 0;
