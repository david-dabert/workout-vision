...
import { createLiftScene, META, liftView } from './lift-scenes';
import './Film.css';
...
export default function Film({ lift, onBack, onFile }) {
  ...
  return <div className="wv-experience">
    <section className="screen is-active film-screen"><div className="wrap">
      ...
      <p className="eyebrow" data-reveal>{fr ? (view === 'side' ? 'Filmé de profil' : 'Filmé de face') : (view === 'side' ? 'Filmed from the side' : 'Filmed from the front')}</p>
      <h2 className="title" data-reveal>{META[lift]?.[lang] || lift}</h2>
      ...
      <p className="caption" data-reveal>{fr ? 'Le cadrage de la série de référence' : 'The framing of the reference set'}</p>
