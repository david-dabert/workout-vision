...
export function interpolateNulls(signal: (number | null)[]): number[] {
  const out: (number | null)[] = [...signal];
  const N = out.length;
...
export function smoothSignal(signal: number[], windowSize: number): number[] {
...
export function medianIntervalFrames(frames: number[]): number {
...
export function findValleys(signal: number[], fps: number, exercise: ExerciseConfig): ValleyResult {
  ...
  const filterWithSpacing = (minGap: number): number[] => {
    const frames = [];
    let last = -Infinity;
...
export function autocorrelationEdgeCorrect(signal: number[], valleyResult: ValleyResult, fps: number): ValleyResult {
  if (valleyResult.reps < 3) return valleyResult;
...
export function templateEdgeCorrect(signal: number[], valleyResult: ValleyResult, fps: number, exercise: ExerciseConfig): ValleyResult {
  if (valleyResult.reps < 3) return valleyResult;
  ...
  let changed = false;
  const diag: Record<string, unknown> = { period, medianDepth: Math.round(medianDepth * 10) / 10, left: null, right: null };
  ...
  if (rightGap >= period * 0.8 && rightGap <= period * 1.2) {
    ...
    if (bestIdx >= 0) {
      ...
      const rightDiag = { gap: rightGap, prom: Math.round(prominence * 10) / 10, val: Math.round(bestVal * 10) / 10, depthMatch, added: false };

      if (prominence >= minAmplitude && depthMatch) {
        newFrames.push(bestIdx);
        changed = true;
        rightDiag.added = true;
      }
      diag.right = rightDiag;
    }
  }
  ...
  if (leftGap >= period * 0.8 && leftGap <= period * 1.2) {
    ...
    if (bestIdx >= 0) {
      ...
      const leftDiag = { gap: leftGap, prom: Math.round(prominence * 10) / 10, val: Math.round(bestVal * 10) / 10, depthMatch, added: false };
